...
results <- apply_naive_bayes(training_set, test_set)
prediction <- results$prediction
prediction_prob <- results$prediction_prob

test_set_reduced <- perform_umap(test_set, set_seed = TRUE)

plot_predictions(test_set_reduced, prediction, "Naive Bayes klasifikavimo rezultatai pilnai aibei")
auc <- roc_curve(test_set, prediction_prob, positive_class = "2", "Naive Bayes ROC kreivė pilnai aibei")
print(auc)
...
results <- apply_naive_bayes(training_set_2d, test_set_2d)
prediction <- results$prediction
prediction_prob <- results$prediction_prob

plot_predictions(test_set_2d, prediction, "Naive Bayes klasifikavimo rezultatai apribotai suspaustai aibei")
auc <- roc_curve(test_set_2d, prediction_prob, positive_class = "2", "Naive Bayes ROC kreivė apribotai suspaustai aibei")
print(auc)